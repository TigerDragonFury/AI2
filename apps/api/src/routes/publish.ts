...
// POST /api/publish
publishRouter.post('/', requireAuth, rateLimiter('publish'), async (req: AuthRequest, res, next) => {
  try {
...
publishRouter.post('/:id/retry', requireAuth, rateLimiter('publish'), async (req: AuthRequest, res, next) => {
  try {
